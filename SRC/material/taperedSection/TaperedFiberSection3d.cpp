...
// $Source: /usr/local/cvs/OpenSees/SRC/material/section/TaperedFiberSection3d.cpp,v $

// Written: fmk
// Created: 04/04
//
// Description: This file contains the class implementation of TaperedFiberSection3d.
...
#include <Fiber.h>
#include <classTags.h>
#include <TaperedFiberSection3d.h>
#include <ID.h>
...
ID TaperedFiberSection3d::code(SEC_TAG_TaperedFiberSection3d);

// constructors:
TaperedFiberSection3d::TaperedFiberSection3d(int tag, int num,
                               Fiber **
                               fibers, double hratio, double g):SectionForceDeformation(tag,
                                                               SEC_TAG_TaperedFiberSection3d),
numFibers(num), theMaterials(0), matData(0), yBar(0.0), zBar(0.0), G(g), hRatio(hratio),
e(4), eCommit(4), s(0), ks(0)
{
    #ifdef DEBUG
	fprintf(stdout, "Inside function TaperedFiberSection3d::TaperedFiberSection3d(...)\n");
    #endif
    ...
    if (numFibers != 0) {
        ...
        if (theMaterials == 0) {
            opserr <<
                "TaperedFiberSection3d::TaperedFiberSection3d -- failed to allocate Material pointers\n";
            exit(-1);
        }
        ...
        if (matData == 0) {
            opserr <<
                "TaperedFiberSection3d::TaperedFiberSection3d -- failed to allocate double array for material data\n";
            exit(-1);
        }
	    ...
	    matData[i * 5 + 4] = tP; // plate thickness for the fiber...this assumes fibers are located at mid-thickness only!
            ...
            if (theMaterials[i] == 0) {
                opserr <<
                    "TaperedFiberSection3d::TaperedFiberSection3d -- failed to get copy of a Material\n";
                exit(-1);
            }
        }
...
TaperedFiberSection3d::TaperedFiberSection3d():
SectionForceDeformation(0, SEC_TAG_TaperedFiberSection3d),
numFibers(0), theMaterials(0), matData(0),
yBar(0.0), zBar(0.0), G(0.0), hRatio(0.0), e(4), eCommit(4), s(0), ks(0)
{
    #ifdef DEBUG
	fprintf(stdout, "Inside function TaperedFiberSection3d::TaperedFiberSection3d()\n");
    #endif
...
int TaperedFiberSection3d::addFiber(Fiber & newFiber)
{
    #ifdef DEBUG
	fprintf(stdout, "Inside function TaperedFiberSection3d::addFiber\n");
    #endif
    ...
    if (newArray == 0 || newMatData == 0) {
        opserr <<
            "TaperedFiberSection3d::addFiber -- failed to allocate Fiber pointers\n";
        exit(-1);
    }
    ...
    if (newArray[numFibers] == 0) {
        opserr <<
            "TaperedFiberSection3d::addFiber -- failed to get copy of a Material\n";
        exit(-1);
...
int TaperedFiberSection3d::setTrialSectionDeformation(const Vector &deforms)
{
    #ifdef DEBUG
	fprintf(stdout, "Inside function TaperedFiberSection3d::setTrialSectionDeformation\n");
	...
	/*
	 * NOTE: For normal TaperedFiberSection3d.
        double strain =
...
const Matrix &TaperedFiberSection3d::getInitialTangent(void)
{
    #ifdef DEBUG
	fprintf(stdout, "Inside function TaperedFiberSection3d::getInitialTangent\n");
    #endif
...
const Vector & TaperedFiberSection3d::getSectionDeformation(void)
{
    return e;
}

const Matrix & TaperedFiberSection3d::getSectionTangent(void)
{
    return *ks;
}

const Vector & TaperedFiberSection3d::getStressResultant(void)
{
    return *s;
}

SectionForceDeformation *TaperedFiberSection3d::getCopy(void)
{
    #ifdef DEBUG
	fprintf(stdout, "Inside function TaperedFiberSection3d::getCopy\n");
    #endif
    TaperedFiberSection3d *theCopy = new TaperedFiberSection3d();
    ...
    if (numFibers != 0) {
        ...
        for (int i = 0; i < numFibers; i++) {
            ...
            if (theCopy->theMaterials[i] == 0) {
                opserr <<
                    "TaperedFiberSection3d::getCopy -- failed to get copy of a Material\n";
                exit(-1);
            }
        }
    }
...
const ID & TaperedFiberSection3d::getType()
{
    return code;
}

int TaperedFiberSection3d::getOrder() const const
{
    return 6; // Initially was a return value of "5"
}

int TaperedFiberSection3d::commitState(void)
{
    #ifdef DEBUG
	fprintf(stdout, "In function TaperedFiberSection3d::commitState\n");
    #endif
...
int TaperedFiberSection3d::revertToLastCommit(void)
{
    #ifdef DEBUG
	fprintf(stdout, "In function TaperedFiberSection3d::revertToLastCommit\n");
    #endif
...
int TaperedFiberSection3d::revertToStart(void)
{
...
int TaperedFiberSection3d::sendSelf(int commitTag, Channel &theChannel)
{
    int res = 0;
...
int TaperedFiberSection3d::recvSelf(int commitTag, Channel & theChannel,
...
void TaperedFiberSection3d::Print(OPS_Stream & s, int flag)
{
    if (flag == 2) {
        for (int i = 0; i < numFibers; i++) {
        ...
        s << "\nTaperedFiberSection3d, tag: " << this->getTag() << endln;
        s << "\tSection code: " << code;
...
Response *TaperedFiberSection3d::setResponse(const char **argv, int argc,
                                      OPS_Stream & output)
{
...
int TaperedFiberSection3d::getResponse(int responseID,
...
int TaperedFiberSection3d::setParameter(const char **argv, int argc,
                                 Parameter & param)
{
    if (argc < 3)
        return -1;
...
const Vector &
    TaperedFiberSection3d::getSectionDeformationSensitivity(int gradIndex)
{
...
const Vector &
    TaperedFiberSection3d::getStressResultantSensitivity(int gradIndex,
                                                  bool conditional)
{
...
const Matrix &
    TaperedFiberSection3d::getSectionTangentSensitivity(int gradIndex)
{
...
int TaperedFiberSection3d::commitSensitivity(const Vector & defSens,
