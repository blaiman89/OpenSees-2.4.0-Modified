			       ...
			       TCL_Char **argv, TclModelBuilder *theTclBuilder)
{
	...
	*/
    ...
    // Brighton Laiman: University of California, San Diego
    else if (strcmp(argv[1], "taperedFiberSec") == 0)
...
buildTaperedSection(Tcl_Interp * interp,
                    ...
                    bool isTorsion, double G, double hRatio);
                                  ...
                                  TclModelBuilder * theTclModelBuilder)
{
    ...
    // additional lines of code added for TAPERED STUDY
    else if (strcmp(argv[3], "-hRatio") == 0) {
	if (Tcl_GetDouble(interp, argv[4], &hRatio) != TCL_OK) {
	    interp->result = "WARNING invalid hRatio";
	    return TCL_ERROR;
	}
	#ifdef DEBUG
	...
	if (strcmp(argv[5], "-G") == 0) {
	    if (Tcl_GetDouble(interp, argv[6], &G) != TCL_OK) {
		interp->result = "WARNING invalid G";
		return TCL_ERROR;
	    }
	    #ifdef DEBUG
	    ...
	    interp->result = "WARNING missing read in for G";
	    return TCL_ERROR;
	} 
	//brace = 5;
	brace = 7;
    }
    // end of additional lines of code
    ...
    if (buildTaperedSection
        (interp, theTclModelBuilder, secTag, isTorsion, G, hRatio) != TCL_OK) {
                           ...
                           TclModelBuilder *theTclModelBuilder)
{
    ...
    if (argc < 6) {
        opserr <<
            "WARNING invalid num args: fiber yLoc zLoc area matTag tP\n";
        return TCL_ERROR;
    }
    ...
    if (Tcl_GetDouble(interp, argv[1], &yLoc) != TCL_OK) {
        opserr <<
            "WARNING invalid yLoc: fiber yLoc zLoc area matTag tP\n";
        return TCL_ERROR;
    }
    if (Tcl_GetDouble(interp, argv[2], &zLoc) != TCL_OK) {
        opserr <<
            "WARNING invalid zLoc: fiber yLoc zLoc area matTag tP\n";
        return TCL_ERROR;
    }
    if (Tcl_GetDouble(interp, argv[3], &area) != TCL_OK) {
        opserr <<
            "WARNING invalid area: fiber yLoc zLoc area matTag tP\n";
        return TCL_ERROR;
    }

    if (Tcl_GetInt(interp, argv[4], &matTag) != TCL_OK) {
        opserr <<
            "WARNING invalid matTag: fiber yLoc zLoc area matTag tP\n";
        return TCL_ERROR;
    }

    if (Tcl_GetDouble(interp, argv[5], &tP) != TCL_OK) {
        opserr <<
            "WARNING invalid matTag: fiber yLoc zLoc area matTag tP\n";
        return TCL_ERROR;
    }
    ...
    if (NDM == 2) {
        ...
        theFiber =
            new UniaxialFiber3d(numFibers, *material, area,
                                fiberPosition, dValue, tP);
        if (theFiber == 0) {
            opserr << "WARNING unable to allocate fiber \n";
            return TCL_ERROR;
        }
        ...
        opserr <<
            "WARNING fiber command for FiberSection only fo 2 or 3d \n";
        return TCL_ERROR;
    }
...
//               opserr<<"HeightValue[i]"<<HeightValue[i];
        ...
        dValue =
            HeightValue[0] - 0.5 * (HeightValue[0] - HeightValue[1]);
        ...
        // copy the section repres fibers
	int count;
        for (count = 0; count < numSectionRepresFibers; count++) 
            ...
            for (i = numSectionRepresFibers; i < numFibers; i++) {
                ...
                fiber[i] =
                    new UniaxialFiber3d(k, *material, fibersArea(k),
                                        fiberPosition, dValue, fiberstP(k));
                if (fibersArea(k) < 0)
	    ...
	    section = new TaperedFiberSectionSmoothing3d(secTag, numFibers, fiber, hRatio, G); 
