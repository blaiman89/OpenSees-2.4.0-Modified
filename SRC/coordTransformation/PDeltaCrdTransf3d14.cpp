...
// $Source: /usr/local/cvs/OpenSees/SRC/coordTransformation/PDeltaCrdTransf3d14.cpp,v $
...
// PDeltaCrdTransf3d14 class. PDeltaCrdTransf3d14 is a linear
...
#include <PDeltaCrdTransf3d14.h>


// constructor:
PDeltaCrdTransf3d14::PDeltaCrdTransf3d14(int tag, const Vector &vecInLocXZPlane):
CrdTransf(tag, CRDTR_TAG_PDeltaCrdTransf3d14),
...
nodeIInitialDisp(0), nodeJInitialDisp(0), initialDispChecked(false)
{
    for (int i = 0; i < 2; i++)
        ...
        if (&rigJntOffset1 == 0 || rigJntOffset1.Size() != 3 ) {
            opserr << "PDeltaCrdTransf3d14::PDeltaCrdTransf3d14:  Invalid rigid joint offset vector for node I\n";
        ...
        if (&rigJntOffset2 == 0 || rigJntOffset2.Size() != 3 ) {
            opserr << "PDeltaCrdTransf3d14::PDeltaCrdTransf3d14:  Invalid rigid joint offset vector for node J\n";
            opserr << "Size must be 3\n";      
        }
...
PDeltaCrdTransf3d14::PDeltaCrdTransf3d14():
CrdTransf(0, CRDTR_TAG_PDeltaCrdTransf3d14),
nodeIPtr(0), nodeJPtr(0),
nodeIOffset(0), nodeJOffset(0),
L(0), ul18(0), ul29(0),
nodeIInitialDisp(0), nodeJInitialDisp(0), initialDispChecked(false)
...
int
PDeltaCrdTransf3d14::commitState(void)
{
    return 0;
}
...
int
PDeltaCrdTransf3d14::revertToLastCommit(void)
{
    return 0;
}
...
int
PDeltaCrdTransf3d14::revertToStart(void)
{
    return 0;
}
...
int 
PDeltaCrdTransf3d14::initialize(Node *nodeIPointer, Node *nodeJPointer)
{       
    int error;
    ...
    {
        opserr << "\nPDeltaCrdTransf3d14::initialize";
...
int
PDeltaCrdTransf3d14::update(void)
{
    ...
    double ul1, ul8, ul2, ul9;
    
    ul1 = R[1][0]*ug[0] + R[1][1]*ug[1] + R[1][2]*ug[2];
    ul2 = R[2][0]*ug[0] + R[2][1]*ug[1] + R[2][2]*ug[2];
    
    ul8 = R[1][0]*ug[7] + R[1][1]*ug[8] + R[1][2]*ug[9];
    ul9 = R[2][0]*ug[7] + R[2][1]*ug[8] + R[2][2]*ug[9];
    ...
    if (nodeJOffset) {
        Wu[0] =  nodeJOffset[2]*ug[11] - nodeJOffset[1]*ug[12];
        Wu[1] = -nodeJOffset[2]*ug[10]  + nodeJOffset[0]*ug[12];
        Wu[2] =  nodeJOffset[1]*ug[10]  - nodeJOffset[0]*ug[11];
        
        ul8 += R[1][0]*Wu[0] + R[1][1]*Wu[1] + R[1][2]*Wu[2];
        ul9 += R[2][0]*Wu[0] + R[2][1]*Wu[1] + R[2][2]*Wu[2];
    }
...
int 
PDeltaCrdTransf3d14::computeElemtLengthAndOrient()
{
    // element projection
    static Vector dx(3);
    ...
    if (L == 0.0) {
        opserr << "\nPDeltaCrdTransf3d14::computeElemtLengthAndOrien: 0 length\n";
        return -2;  
    }
...
int
PDeltaCrdTransf3d14::getLocalAxes(Vector &XAxis, Vector &YAxis, Vector &ZAxis)
{
    // Compute y = v cross x
    ...
    if (ynorm == 0) {
        opserr << "\nPDeltaCrdTransf3d14::getLocalAxes";
...
double 
PDeltaCrdTransf3d14::getInitialLength(void)
{
    return L;
}
...
double 
PDeltaCrdTransf3d14::getDeformedLength(void)
{
    return L;
}
...
const Vector &
PDeltaCrdTransf3d14::getBasicTrialDisp (void)
{
    ...
    if (nodeJOffset) {
        ...
        ul[7] += R[0][0]*Wu[0] + R[0][1]*Wu[1] + R[0][2]*Wu[2];
        ul[8] += R[1][0]*Wu[0] + R[1][1]*Wu[1] + R[1][2]*Wu[2];
        ul[9] += R[2][0]*Wu[0] + R[2][1]*Wu[1] + R[2][2]*Wu[2];
    }
...
const Vector &
PDeltaCrdTransf3d14::getBasicIncrDisp (void)
{
    // determine global displacements
...
const Vector &
PDeltaCrdTransf3d14::getBasicIncrDeltaDisp(void)
{
    // determine global displacements
...
const Vector &
PDeltaCrdTransf3d14::getBasicTrialVel(void)
{
	...
	if (nodeJOffset) {
		Wu[0] =  nodeJOffset[2]*vg[11] - nodeJOffset[1]*vg[12];
		Wu[1] = -nodeJOffset[2]*vg[10]  + nodeJOffset[0]*vg[12];
		Wu[2] =  nodeJOffset[1]*vg[10]  - nodeJOffset[0]*vg[11];
		
		vl[7] += R[0][0]*Wu[0] + R[0][1]*Wu[1] + R[0][2]*Wu[2];
		vl[8] += R[1][0]*Wu[0] + R[1][1]*Wu[1] + R[1][2]*Wu[2];
		vl[9] += R[2][0]*Wu[0] + R[2][1]*Wu[1] + R[2][2]*Wu[2];
	}
...
const Vector &
PDeltaCrdTransf3d14::getBasicTrialAccel(void)
{
	...
	if (nodeJOffset) {
		Wu[0] =  nodeJOffset[2]*ag[11] - nodeJOffset[1]*ag[12];
		Wu[1] = -nodeJOffset[2]*ag[10]  + nodeJOffset[0]*ag[12];
		Wu[2] =  nodeJOffset[1]*ag[10]  - nodeJOffset[0]*ag[11];
		
		al[7] += R[0][0]*Wu[0] + R[0][1]*Wu[1] + R[0][2]*Wu[2];
		al[8] += R[1][0]*Wu[0] + R[1][1]*Wu[1] + R[1][2]*Wu[2];
		al[9] += R[2][0]*Wu[0] + R[2][1]*Wu[1] + R[2][2]*Wu[2];
	}
...
const Vector &
PDeltaCrdTransf3d14::getGlobalResistingForce(const Vector &pb, const Vector &p0)
{
	...
	// Element loads do not work; not implemented
    pl[0] += p0(0);
    pl[1] += p0(1);
    pl[7] += p0(2);
    pl[2] += p0(3);
    ...
    double NoverL;
    NoverL = ul18*q8*oneOverL;             
    pl[1] += NoverL;
    pl[8] -= NoverL;
    NoverL = ul29*q8*oneOverL;
    pl[2] += NoverL;
    pl[9] -= NoverL;
    ...
    if (nodeJOffset) {
        pg(10)  += -nodeJOffset[2]*pg(8) + nodeJOffset[1]*pg(9);
        pg(11) +=  nodeJOffset[2]*pg(7) - nodeJOffset[0]*pg(9);
        pg(12) += -nodeJOffset[1]*pg(7) + nodeJOffset[0]*pg(8);
    }
...
const Matrix &
PDeltaCrdTransf3d14::getGlobalStiffMatrix (const Matrix &KB, const Vector &pb)
{
    ...
    for (i = 0; i < 9; i++)
        ...
        for (m = 0; m < 14; m++) {
            ...
            if (nodeJOffset) {
                tmp[m][10]   += kl[m][7]*RWJ[0][0]  + kl[m][8]*RWJ[1][0]  + kl[m][9]*RWJ[2][0];
                tmp[m][11]  += kl[m][7]*RWJ[0][1]  + kl[m][8]*RWJ[1][1]  + kl[m][9]*RWJ[2][1];
                tmp[m][12]  += kl[m][7]*RWJ[0][2]  + kl[m][8]*RWJ[1][2]  + kl[m][9]*RWJ[2][2];
            }
            tmp[m][13] = kl[m][13];
        }
        ...
        for (m = 0; m < 14; m++) {
            ...
            if (nodeJOffset) {
                kg(10,m)  += RWJ[0][0]*tmp[7][m]  + RWJ[1][0]*tmp[8][m] + RWJ[2][0]*tmp[9][m];
                kg(11,m) += RWJ[0][1]*tmp[7][m]  + RWJ[1][1]*tmp[8][m] + RWJ[2][1]*tmp[9][m];
                kg(12,m) += RWJ[0][2]*tmp[7][m]  + RWJ[1][2]*tmp[8][m] + RWJ[2][2]*tmp[9][m];
            }
			kg(13,m) = tmp[13][m];
        }
...
const Matrix &
PDeltaCrdTransf3d14::getInitialGlobalStiffMatrix (const Matrix &KB)
{
    static Matrix kg(14,14);	// Global stiffness for return
    static double kb[9][9];		// Basic stiffness
    static double kl[14][14];	// Local stiffness
...
CrdTransf *
PDeltaCrdTransf3d14::getCopy3d(void)
{
    // create a new instance of PDeltaCrdTransf3d14 
    
    PDeltaCrdTransf3d14 *theCopy;
    ...
    theCopy = new PDeltaCrdTransf3d14(this->getTag(), xz, offsetI, offsetJ);
...
int 
PDeltaCrdTransf3d14::sendSelf(int cTag, Channel &theChannel)
{
    int res = 0;
    ...
    if (nodeJInitialDisp != 0) {
        data(15) = nodeJInitialDisp[0];
        data(16) = nodeJInitialDisp[1];
        data(17) = nodeJInitialDisp[2];
        data(18) = nodeJInitialDisp[3];
        data(19) = nodeJInitialDisp[4];
        data(20) = nodeJInitialDisp[5];
		data(21) = nodeJInitialDisp[6];
    } else {
        data(15) = 0.0;
        data(16) = 0.0;
        data(17) = 0.0;
    ...
    if (res < 0) {
        opserr << "PDeltaCrdTransf3d14::sendSelf - failed to send Vector\n";
        return res;
    }
...
int 
PDeltaCrdTransf3d14::recvSelf(int cTag, Channel &theChannel, FEM_ObjectBroker &theBroker)
{
    int res = 0;
    ...
    if (res < 0) {
        opserr << "PDeltaCrdTransf3d14::recvSelf - failed to receive Vector\n";
        return res;
    }
...
const Vector &
PDeltaCrdTransf3d14::getPointGlobalCoordFromLocal(const Vector &xl)
{
    static Vector xg(3);
...
const Vector &
PDeltaCrdTransf3d14::getPointGlobalDisplFromBasic (double xi, const Vector &uxb)
{
    // determine global displacements
    const Vector &disp1 = nodeIPtr->getTrialDisp();
    const Vector &disp2 = nodeJPtr->getTrialDisp();
...
void
PDeltaCrdTransf3d14::Print(OPS_Stream &s, int flag)
{
    s << "\nCrdTransf: " << this->getTag() << " Type: PDeltaCrdTransf3d14" << endln;
    if (nodeIOffset)
